...
use Spatie\WordPressRay\Composer\Console\Application as ComposerApplication;
use Spatie\WordPressRay\Composer\Factory;
...
use Spatie\WordPressRay\Symfony\Component\Console\Input\StringInput;
use Spatie\WordPressRay\Symfony\Component\Console\Output\OutputInterface;
use Spatie\WordPressRay\Composer\Command\BaseCommand;
use Spatie\WordPressRay\Composer\Json\JsonFile;
class BinCommand extends BaseCommand
{
    ...
    private function executeInNamespace(ComposerApplication $application, $namespace, InputInterface $input, OutputInterface $output)
    {
        ...
        $this->getIO()->writeError('<info>Run with <comment>' . $input->__toString() . '</comment></info>', \true, IOInterface::VERBOSE);
    ...
    {
        \chdir($dir);
        $this->getIO()->writeError('<info>Changed current directory to ' . $dir . '</info>', \true, IOInterface::VERBOSE);
    }
