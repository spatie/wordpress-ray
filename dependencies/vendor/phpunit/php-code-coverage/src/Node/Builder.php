 ...
 */
namespace Spatie\WordPressRay\SebastianBergmann\CodeCoverage\Node;
...
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\ProcessedCodeCoverageData;
    ...
    private function addItems(Directory $root, array $items, array $tests) : void
    {
        foreach ($items as $key => $value) {
            ...
            if (substr($key, -2) === '/f') {
                ...
                if (is_file($filename)) {
                    $root->addFile(new File($key, $root, $value['lineCoverage'], $value['functionCoverage'], $tests, $this->coveredFileAnalyser->classesIn($filename), $this->coveredFileAnalyser->traitsIn($filename), $this->coveredFileAnalyser->functionsIn($filename), $this->coveredFileAnalyser->linesOfCodeFor($filename)));
                }
            } else {
