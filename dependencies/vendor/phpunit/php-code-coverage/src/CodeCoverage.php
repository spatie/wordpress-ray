 ...
 */
namespace Spatie\WordPressRay\SebastianBergmann\CodeCoverage;
...
use Spatie\WordPressRay\PHPUnit\Util\Test;
use ReflectionClass;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\Driver\Driver;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\Node\Builder;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\Node\Directory;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\StaticAnalysis\CachingCoveredFileAnalyser;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\StaticAnalysis\CachingUncoveredFileAnalyser;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\StaticAnalysis\CoveredFileAnalyser;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\StaticAnalysis\ParsingCoveredFileAnalyser;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\StaticAnalysis\ParsingUncoveredFileAnalyser;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\StaticAnalysis\UncoveredFileAnalyser;
...
final class CodeCoverage
{
    ...
    private function addUncoveredFilesFromFilter() : void
    {
        ...
        foreach ($uncoveredFiles as $uncoveredFile) {
            if (is_file($uncoveredFile)) {
                $this->append(RawCodeCoverageData::fromUncoveredFile($uncoveredFile, $this->uncoveredFileAnalyser()), self::UNCOVERED_FILES);
            }
        }
    }
    /**
    ...
    private function coveredFileAnalyser() : CoveredFileAnalyser
    {
        ...
        $this->coveredFileAnalyser = new ParsingCoveredFileAnalyser($this->useAnnotationsForIgnoringCode, $this->ignoreDeprecatedCode);
        if ($this->cachesStaticAnalysis()) {
            $this->coveredFileAnalyser = new CachingCoveredFileAnalyser($this->cacheDirectory, $this->coveredFileAnalyser);
        }
        return $this->coveredFileAnalyser;
    }
    private function uncoveredFileAnalyser() : UncoveredFileAnalyser
    {
        ...
        if ($this->cachesStaticAnalysis()) {
            $this->uncoveredFileAnalyser = new CachingUncoveredFileAnalyser($this->cacheDirectory, $this->uncoveredFileAnalyser);
        }
        return $this->uncoveredFileAnalyser;
    }
}
