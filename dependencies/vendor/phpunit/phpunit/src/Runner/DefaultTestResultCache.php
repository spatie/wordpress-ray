...
final class DefaultTestResultCache implements Serializable, TestResultCache
{
     ...
     * @var int[]
     */
    private const ALLOWED_CACHE_TEST_STATUSES = [BaseTestRunner::STATUS_SKIPPED, BaseTestRunner::STATUS_INCOMPLETE, BaseTestRunner::STATUS_FAILURE, BaseTestRunner::STATUS_ERROR, BaseTestRunner::STATUS_RISKY, BaseTestRunner::STATUS_WARNING];
    /**
    ...
    public function saveToFile() : void
    {
        ...
        file_put_contents($this->cacheFilename, serialize($this));
    }
    ...
    public function load() : void
    {
        ...
        $cache = ErrorHandler::invokeIgnoringWarnings(static function () use($cacheData) {
    ...
    {
        return serialize(['defects' => $this->defects, 'times' => $this->times]);
    }
    /**
    ...
    public function unserialize($serialized) : void
    {
        ...
        if (isset($data['defects'])) {
            foreach ($data['defects'] as $testName => $testResult) {
                ...
                if (in_array($testResult, self::ALLOWED_CACHE_TEST_STATUSES, \true)) {
