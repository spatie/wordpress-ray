...
class CliTestDoxPrinter extends TestDoxPrinter
{
    ...
    {
        return trim(\Spatie\WordPressRay\PHPUnit\Framework\TestFailure::exceptionToString($t));
    }
    ...
    {
        $trace = \Spatie\WordPressRay\PHPUnit\Util\Filter::getFilteredStacktrace($t);
        if (!$this->colors) {
            return $trace;
        }
        $lines = [];
