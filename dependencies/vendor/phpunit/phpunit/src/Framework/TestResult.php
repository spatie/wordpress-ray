...
use Countable;
use Error;
use Spatie\WordPressRay\PHPUnit\Util\ErrorHandler;
use Spatie\WordPressRay\PHPUnit\Util\ExcludeList;
...
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\Exception as OriginalCodeCoverageException;
use Spatie\WordPressRay\SebastianBergmann\CodeCoverage\UnintentionallyCoveredCodeException;
use Spatie\WordPressRay\SebastianBergmann\Invoker\Invoker;
use Spatie\WordPressRay\SebastianBergmann\Invoker\TimeoutException;
...
final class TestResult implements Countable
{
    ...
    public function endTest(Test $test, float $time) : void
    {
        ...
        if (!$this->lastTestFailed && $test instanceof TestCase) {
            ...
            $this->passed[$key] = ['result' => $test->getResult(), 'size' => \Spatie\WordPressRay\PHPUnit\Util\Test::getSize($class, $test->getName(\false))];
            $this->time += $time;
        }
        if ($this->lastTestFailed && $test instanceof TestCase) {
    ...
    public function run(Test $test) : void
    {
        ...
        $monitorFunctions = $this->beStrictAboutResourceUsageDuringSmallTests && !$test instanceof ErrorTestCase && !$test instanceof WarningTestCase && $test->getSize() === \Spatie\WordPressRay\PHPUnit\Util\Test::SMALL && function_exists('xdebug_start_function_monitor');
        if ($monitorFunctions) {
        ...
        try {
            $invoker = new Invoker();
            if (!$test instanceof ErrorTestCase && !$test instanceof WarningTestCase && $this->enforceTimeLimit && ($this->defaultTimeLimit || $test->getSize() != \Spatie\WordPressRay\PHPUnit\Util\Test::UNKNOWN) && $invoker->canInvokeWithTimeout()) {
                switch ($test->getSize()) {
                    case \Spatie\WordPressRay\PHPUnit\Util\Test::SMALL:
                        $_timeout = $this->timeoutForSmallTests;
                        break;
                    case \Spatie\WordPressRay\PHPUnit\Util\Test::MEDIUM:
                        $_timeout = $this->timeoutForMediumTests;
                        break;
                    case \Spatie\WordPressRay\PHPUnit\Util\Test::LARGE:
                        $_timeout = $this->timeoutForLargeTests;
                        break;
                    case \Spatie\WordPressRay\PHPUnit\Util\Test::UNKNOWN:
        ...
        if ($collectCodeCoverage) {
            ...
            if ($append && $test instanceof TestCase) {
                try {
                    $linesToBeCovered = \Spatie\WordPressRay\PHPUnit\Util\Test::getLinesToBeCovered(get_class($test), $test->getName(\false));
                    $linesToBeUsed = \Spatie\WordPressRay\PHPUnit\Util\Test::getLinesToBeUsed(get_class($test), $test->getName(\false));
