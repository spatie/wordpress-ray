...
class Backtrace
{
    ...
    protected function toFrameObjects(array $rawFrames) : array
    {
        ...
        foreach ($rawFrames as $rawFrame) {
            $frames[] = new Frame($currentFile, $currentLine, $rawFrame['args'] ?? null, $rawFrame['function'] ?? null, $rawFrame['class'] ?? null, $this->isApplicationFrame($currentFile));
            $currentFile = $rawFrame['file'] ?? 'unknown';
            $currentLine = $rawFrame['line'] ?? 0;
        }
        $frames[] = new Frame($currentFile, $currentLine, [], '[top]');
